...
import { syncLiveGame, saveEndGame, saveEditGame } from '../data/api';
...
export default function GameScreen() {
  ...
  useEffect(() => {
    ...
    function onKeyDown(e) {
      ...
      if (pendingAction) {
        ...
        let ordered;
        if (isSubFlow) {
          if (pendingAction.outIndex != null) ordered = benchPlayers;
          else if (pendingAction.inIndex != null) ordered = courtPlayers;
          else ordered = [...courtPlayers, ...benchPlayers];
        } else {
          ordered = courtPlayers;
        }
        ...
        if (isSubFlow) {
          if (pendingAction.outIndex != null) {
            handleSubIn(pendingAction.side, actualIndex);
          } else if (pendingAction.inIndex != null) {
            handleSubOut(pendingAction.side, actualIndex);
          } else if (pIdx < courtPlayers.length) {
            handleSubOut(pendingAction.side, actualIndex);
          } else {
            handleSubIn(pendingAction.side, actualIndex);
          }
  ...
  useEffect(() => {
    ...
    const saveFn = game.editMode ? saveEditGame : saveEndGame;
    saveFn(
      bs,
      ...
      game.selectedSeason?.id,
      {
        ...
        customGame: !game.editMode && !game.selectedGame,
      },
    )
          ...
          <div className="player-grid">
            {courtPlayers.map((player, displayIdx) => {
              ...
              return renderPlayerCard(player, team, {
                ...
                hotkey: subPicked !== 'out' ? PLAYER_HOTKEYS[displayIdx] : null,
              });
            })}
          </div>
          ...
          <div className="player-grid">
            {benchPlayers.length > 0 ? benchPlayers.map((player, displayIdx) => {
              ...
              return renderPlayerCard(player, team, {
                ...
                hotkey: subPicked === 'none'
                  ? PLAYER_HOTKEYS[courtPlayers.length + displayIdx]
                  : (subPicked === 'out' ? PLAYER_HOTKEYS[displayIdx] : null),
              });
            }) : (
    ...
    <div className={`screen game-screen ${correctionMode ? 'correction-mode' : ''}`}>
      ...
      <div className="game-scoreboard">
        <div className="scoreboard-team home">
          ...
          {bs.teamInfo.home.score.target != null && (
            <span className="scoreboard-target" style={{ fontSize: '0.7em', opacity: 0.7, marginLeft: '8px' }}>
              🎯 {bs.teamInfo.home.score.target}
            </span>
          )}
        </div>
        <div className="scoreboard-clock">
          <span className="scoreboard-quarter">{formatQuarter(quarter)}</span>
          {game.editMode && bs.teamInfo.home.score.targetPerQuarter && (() => {
            const qKey = quarter === 1 ? 'first' : quarter === 2 ? 'second' : quarter === 3 ? 'third' : quarter === 4 ? 'fourth' : null;
            if (!qKey) return null;
            const hT = bs.teamInfo.home.score.targetPerQuarter[qKey];
            const aT = bs.teamInfo.away.score.targetPerQuarter[qKey];
            if (hT == null && aT == null) return null;
            return (
              <span className="scoreboard-q-target" style={{ fontSize: '0.65em', opacity: 0.7, display: 'block' }}>
                🎯 Q{quarter}: {hT ?? 0}-{aT ?? 0}
              </span>
            );
          })()}
          ...
          {game.editMode && !isFinal && (
            <button
              className="btn btn-small btn-end-game"
              style={{ background: '#7a2a2a', marginTop: '4px' }}
              onClick={() => {
                const ok = window.confirm(
                  'Wipe all stats and team scores? Original scores will be preserved as 🎯 targets in the UI. This resets the clock to Q1 so you can re-track from the start.'
                );
                if (ok) dispatch({ type: 'WIPE_STATS' });
              }}
            >
              WIPE STATS
            </button>
        ...
        <div className="scoreboard-team away">
          {bs.teamInfo.away.score.target != null && (
            <span className="scoreboard-target" style={{ fontSize: '0.7em', opacity: 0.7, marginRight: '8px' }}>
              🎯 {bs.teamInfo.away.score.target}
            </span>
          )}
