// Move Lakers vs Los Baggos R4 game from 19:00 C3 to 19:00 C1, swap TBD to C3.
...
(async function () {
    ...
    var game = (day2.games || []).find(function (g) { return !g.isPlaceholder && g.time === "19:00" && g.court === 3 && g.home === "Lakers" && g.away === "Los Baggos"; });
    if (!game) { console.error("not found"); process.exit(1); }
    var c1Slot = (day2.games || []).find(function (g) { return g.time === "19:00" && g.court === 1; });
    if (c1Slot && !c1Slot.isPlaceholder) { console.error("C1 occupied"); process.exit(1); }
    game.court = 1;
    if (c1Slot) { c1Slot.court = 3; }
    ...
    console.log("MOVED Lakers vs Los Baggos C3 → C1; TBD now at C3");
